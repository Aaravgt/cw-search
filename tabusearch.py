...
def corr_energy_numba(a: np.ndarray) -> int:
    n = a.shape[0]
    m = n // 2
    e = 0
...
def best_1swap_local(a:np.ndarray, max_candidates=None):
    """
...
def best_2swap_local(a:np.ndarray, max_candidates=None):
    """
...
def path_relink_preserving(A:np.ndarray, B:np.ndarray, max_steps=200):
    """
...
def tabu_search_cw(
    ...
    """
    n = len(a0)
    current = np.asarray(a0,dtype=np.int8)
    ...
    elite = [best.copy()]      # archive of best solutions
...
def main():
    ...
    for n,ksq in tests:
        k = int(math.isqrt(ksq))
        arr = make_grouped_seed(n,k)
        print(f"CW{n,ksq}'s random array is {arr}")
        ls1swaplimit=5000 if n>32. else None  # e.g. None for n=24, or 5000 for n=48
        ls_2swaplimit=1000 if n> 32 else None
        implimit = 10000 if n>32 else 1000
        best= tabu_search_cw(
            arr,
            tabu_tenure=(2*k),
            no_improve_limit=implimit,
            ls_1swap_limit=ls1swaplimit,
            ls_2swap_limit=ls_2swaplimit)
        print(best)
        valid = validate_cw(best["best_seq"],k)
        print(f"Valid CW -> {valid.valid}\n")
